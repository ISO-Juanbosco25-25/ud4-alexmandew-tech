# 🔧 PROPUESTAS DE MEJORA DEL PROCESO DE MONTAJE

## 👨‍💻 Alumno:Alejandro Robla Arias
## 🖥️ Equipo:

---

## 🎯 OBJETIVO

Proponer mejoras en el proceso de montaje del equipo informático, basándose en los errores detectados y en la experiencia adquirida durante la práctica.

---

## ⚠️ INSTRUCCIONES

Debes proponer al menos **3 mejoras reales** del proceso de montaje.

Las mejoras deben ser:
- Claras
- Aplicables en un entorno real
- Relacionadas con problemas detectados o posibles

---

# 🟢 MEJORA 1

📌 ¿Qué mejorarías?
(Describe el cambio que harías)
El orden al montar los componentes

📌 ¿Por qué?
(Explica el motivo)
Porque me estorbaron algunos componentes y cables al colocarlos en el orden incorrecto.

📌 ¿Cómo lo harías mejor?
(Explica el proceso correcto)
Memorizando el orden debido de montaje de los componentes

📌 ¿Qué beneficio tiene?
(Ejemplo: evitar errores, mejorar rendimiento, mayor seguridad…)
Mayor comodidad al montar y mayor seguridad

---

# 🟢 MEJORA 2

📌 ¿Qué mejorarías?
La protección al montarlo

📌 ¿Por qué?
Porque algunos componentes pueden cortar o crear heridas
📌 ¿Cómo lo harías mejor?
Usando guantes
📌 Beneficio:
Mayor seguridad

---

# 🟢 MEJORA 3

📌 ¿Qué mejorarías?
La cantidad de cables
📌 ¿Por qué?
Porque hay muchos cables innecesarios que estorban al instalar los componentes
📌 ¿Cómo lo harías mejor?
Reduciendo la cantidad de cables en cada componente
📌 Beneficio:
Mayor comodidad

---

# 🧠 REFLEXIÓN FINAL

- ¿Crees que montar un equipo requiere experiencia? ¿Por qué?
  No. Solo necesitas un poco de experiencia pero no mucha.
- ¿Qué harías diferente si tuvieras que montar otro equipo desde cero?
Cambiar el orden de montaje del equipo.
- ¿Te ves capaz de montar un equipo tú solo/a? Justifica tu respuesta.
  Aún creo que no. No me siento muy familiarizado con muchos tipos de equipos.
